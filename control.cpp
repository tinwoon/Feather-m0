...
//static STEP_MOTOR step_motor_one_init{ 1, 1, 1, INT_MAX, 2000};
//static STEP_MOTOR step_motor_two_init{ 2, 1, 1, INT_MAX, 4000};
//static STEP_MOTOR step_motor_three_init{ 3, 1, 1, 200, 2000};
        ...
        case PARAMETER_STEP_CONTROL_ALL:
            ...
            //set angle and rotation_number and rpm
            {
                ...
                while(motor_step_count[0] || motor_step_count[1] || motor_step_count[2]){
                    ...
                    //delay에 따라 동작이 달라지지만 세 모터의 속도는 가장 먼저 동작된 모터의 속도를 따른다.
                    delayMicroseconds(step_motor_control_data[motor_id_index].step_delay);
